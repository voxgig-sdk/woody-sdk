...
[MCP](https://modelcontextprotocol.io) server exposing the Woody SDK as
two agent tools — `woody_list` and `woody_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/woody-mcp)
make build

# 2. Provide credentials via the environment
export WOODY_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user woody \
  -- /absolute/path/to/woody-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./woody-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// woody_list: first page of records
{ "entity": "api" }
{ "entity": "api", "query": { } }

// woody_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/woody-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user woody \
     -- "$PWD"/dist/*/woody-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `woody_list` and `woody_load` tools now appear
   in new sessions. Ask the agent to *"list api using woody"*
   and it calls `woody_list` with `{"entity":"api"}`.
...
```sh
export WOODY_APIKEY=sk_live_xxx            # API key
export WOODY_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `woody_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `woody_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `WOODY_APIKEY` | API key sent with every request. |
| `WOODY_BASE` | Optional override of the API base URL. |
...
```sh
./woody-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"woody_load","arguments":{"entity":"api","query":{"id":1}}}}'
```
