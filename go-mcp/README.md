...
MCP server exposing the Woody SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `woody_list` | `entity`, optional `query` map | First page of records as JSON |
| `woody_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `woody_list` and
`woody_load` tools become available in new sessions.
