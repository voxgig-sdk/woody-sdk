...
The Ruby SDK for the Woody API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
client = WoodySDK.test(testopts, sdkopts)
```
...
raise a `WoodyError` (a `StandardError` subclass), so wrap
...
The main module (`Woody_sdk`) exports the SDK class
