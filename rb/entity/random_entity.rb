...
class RandomEntity
  ...
  def data_set(args)
    if args
      @_data = WoodyHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = WoodyHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Random, Hash] the loaded Random; raises WoodyError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = WoodyHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
