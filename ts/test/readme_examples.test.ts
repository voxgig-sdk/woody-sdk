...
//     (\`new WoodySDK(...)\` / \`WoodySDK.test(...)\` become
//     \`WoodySDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"api":{"test01":{"id":"test01"}},"random":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new WoodySDK(...)\` and any
// \`WoodySDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+WoodySDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/WoodySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
