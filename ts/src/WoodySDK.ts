...
class WoodySDK {
  ...
  async direct(fetchargs?: any) {
    if (!this._options.allow.op.includes('direct')) {
      return {
        ok: false,
        err: new Error('WoodySDK: direct: operation not allowed by' +
  ...
  async graphql(query: string, variables?: any, ctrl?: any) {
    ...
    if (!options.allow.op.includes('graphql')) {
      return {
        ok: false,
        err: new Error('WoodySDK: graphql: operation not allowed by' +
    ...
    if (null != errors && Array.isArray(errors) && 0 < errors.length) {
      ...
      const err: any = new Error('WoodySDK: graphql: ' +
  ...
  static test(testoptsarg?: any, sdkoptsarg?: any) {
    ...
    const testsdk = new WoodySDK(sdkopts)
    testsdk._mode = 'test'
  ...
  tester(testopts?: any, sdkopts?: any) {
    return WoodySDK.test(testopts, sdkopts)
  }
  ...
  toString() {
    return 'Woody ' + this._utility.struct.jsonify(this.toJSON())
  }
