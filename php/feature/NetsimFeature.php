...
class WoodyNetsimFeature extends WoodyBaseFeature
{
    ...
    private function _simulate(WoodyContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(WoodyContext $ctx, array $applied): void
    {
        $client = $this->client;
