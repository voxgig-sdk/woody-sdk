...
class FtCtrl extends WoodyControl
{
...
class FtClient
{
    ...
    private WoodyUtility $utility;

    public function __construct(WoodyUtility $utility, array $options)
    {
    ...
    public function get_utility(): WoodyUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WoodyContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (WoodyContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WoodyConfig::make_feature($name);
    ...
    private static function buildUrl(WoodySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WoodyError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WoodyResponse($fetched);
        }
    ...
    private function fail(WoodyContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WoodyContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
