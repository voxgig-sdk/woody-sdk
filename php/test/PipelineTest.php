...
{
    private static function utility(): WoodyUtility
    {
        return new WoodyUtility();
    }
    ...
    private static function ctx(array $over = []): WoodyContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new WoodyContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new WoodyOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof WoodyError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new WoodyOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = WoodyMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new WoodyOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = WoodyMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new WoodyOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = WoodyMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new WoodySpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = WoodyMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new WoodyResponse([]),
            'result' => new WoodyResult([]),
        ]);
        [, $err] = WoodyMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new WoodySpec(['step' => 's']),
            'response' => new WoodyResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new WoodyResult([]),
        ]);
        [, $err] = WoodyMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new WoodySpec(['step' => 's']),
            'response' => new WoodyResponse(self::resp(200, ['v' => 1])),
            'result' => new WoodyResult([]),
        ]);
        [, $err] = WoodyMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new WoodySpec(['step' => 's']),
            'response' => new WoodyResponse(self::resp(200, ['v' => 2])),
            'result' => new WoodyResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        WoodyMakeResponse::call($ctx);
    ...
    {
        $preset = new WoodyResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new WoodySpec([]),
            'response' => new WoodyResponse([]),
            'result' => new WoodyResult([]),
        ...
        [$got, $err] = WoodyMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new WoodyResult([])]);
        [, $err] = WoodyMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new WoodySpec([]), 'result' => null]);
        [, $err] = WoodyMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new WoodySpec(['step' => 's']),
            'result' => new WoodyResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new WoodyOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = WoodyMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new WoodySpec(['step' => 's']),
            'result' => new WoodyResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new WoodyOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = WoodyMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new WoodyResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new WoodySpec([]),
            'result' => new WoodyResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = WoodyMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new WoodyError('boom', 'boom');
        $utility->fetcher = function (WoodyContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new WoodySpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = WoodyMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (WoodyContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = WoodyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(WoodyResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (WoodyContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new WoodySpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = WoodyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new WoodyResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new WoodySpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = WoodyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = WoodyMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = WoodyMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new WoodyResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, WoodyDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new WoodyResult(['ok' => false])]);
        $this->expectException(WoodyError::class);
        WoodyDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new WoodyResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', WoodyMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new WoodyBaseFeature();
        $b = new WoodyBaseFeature();
        WoodyFeatureAdd::call($ctx, $a);
        WoodyFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): WoodyBaseFeature
    {
        $f = new WoodyBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): WoodyContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new WoodySpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new WoodyResponse(['headers' => null]),
            'result' => new WoodyResult([]),
        ]);
        WoodyResultHeaders::call($ctx);
