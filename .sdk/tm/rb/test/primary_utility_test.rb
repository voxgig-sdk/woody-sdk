...
class PrimaryUtilityTest < Minitest::Test
  ...
  def test_feature_hook_basic
    hook_client = WoodySDK.test(nil, nil)
  ...
  def test_feature_init_basic
    init_client = WoodySDK.test(nil, nil)
  ...
  def test_make_result_no_result
    ctx = make_test_full_ctx(@client, @utility)
    ctx.spec = WoodySpec.new({ "step" => "start" })
    ctx.result = nil
  ...
  def test_make_spec_basic
    ...
    spec_client = WoodySDK.test(nil, setup_opts)
  ...
  def test_make_url_basic
    runset(get_spec(@primary, "makeUrl", "basic")) do |entry|
      ctxmap = entry["ctx"] || {}
      ctx = make_ctx_from_map(ctxmap, @client, @utility)
      ctx.result = WoodyResult.new({}) unless ctx.result
  ...
  def test_prepare_auth_basic
    ...
    auth_client = WoodySDK.test(nil, setup_opts)
  ...
  def make_ctx_from_map(ctxmap, client, utility)
    ...
    ctx = WoodyContext.new(ctxmap, nil)
    ...
    if ctxmap["response"].is_a?(Hash)
      ...
      ctx.response = WoodyResponse.new(resp_map)
      if resp_map["body"]
  ...
  def err_from_map(m)
    ...
    code = m["code"] || ""
    WoodyError.new(code, msg)
  end
end


# === Test hook feature for featureHook test ===
class TestHookFeature < WoodyBaseFeature
...
class TestInitFeature < WoodyBaseFeature
