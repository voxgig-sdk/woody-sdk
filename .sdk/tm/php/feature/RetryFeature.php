...
class WoodyRetryFeature extends WoodyBaseFeature
{
    ...
    public function init(WoodyContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (WoodyContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(WoodyContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(WoodyContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
