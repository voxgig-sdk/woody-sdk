...
boru-driven command-line client **and** interactive REPL for the Woody
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/woody-cli)
make build
...
./woody-cli load 1 api            # {id:1} shorthand
./woody-cli load '{id:1}' api       # explicit match map

# 5. Override the API base URL for a single call
WOODY_BASE=https://api.example.com ./woody-cli load 1 api
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/woody-cli
   ```
...
```sh
./woody-cli load 1 api          # scalar shorthand for {id:1}
./woody-cli load '{id:1}' api     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `woody>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/woody-cli`. |
...
(`./woody-cli <expr>`) and interactively (the REPL), and expressions compose the
